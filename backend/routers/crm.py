...
async def get_valid_access_token() -> str:
    try:
        ...
        raise HTTPException(status_code=401, detail="Unable to authenticate with HubSpot")
...
async def create_hubspot_lead(lead: Lead):
    try:
        ...
        logger.error(f"Unexpected error creating lead in HubSpot: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Unexpected error creating lead: {str(e)}")
